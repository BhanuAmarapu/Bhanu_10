...
start "ML Service (FastAPI)" cmd /k "cd ml-service && (if exist ..\.venv\Scripts\python.exe (..\.venv\Scripts\python -m uvicorn main:app --host 127.0.0.1 --port 8000) else (python -m uvicorn main:app --host 127.0.0.1 --port 8000))"

:: 2. Start Backend Flask Server in a new window
echo [2/3] Starting Backend Server (Flask) on port 5000...
start "Backend Server (Flask)" cmd /k "cd backend && (if exist ..\.venv\Scripts\python.exe (..\.venv\Scripts\python run.py) else (python run.py))"
